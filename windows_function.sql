...
from(
SELECT category_id, film_id, title, rental_count,RANK() OVER (PARTITION BY category_id ORDER BY rental_count DESC) AS rank_in_category
FROM (
...
SELECT category_id, film_id, title, rental_count,
       SUM(rental_count) OVER (PARTITION BY category_id ORDER BY rental_count) AS running_total
FROM (
    SELECT fc.category_id, f.film_id, f.title, COUNT(r.rental_id) AS rental_count
    FROM film f
    JOIN film_category fc ON f.film_id = fc.film_id
...
SELECT film_id,title,category,rental_count,avg_rental_count
